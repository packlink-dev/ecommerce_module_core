---
description: Scaffold a new .ai-docs/change_requests/<CR-ID> folder (spec.md, design.md, tasks.md, optional requirements/)
---

Scaffold a new change-request folder under `.ai-docs/change_requests/`, following the structure
documented in `.ai-docs/guidelines.md`.
...
1. Read `.ai-docs/guidelines.md` for the current convention if you haven't already this session.
2. Create `.ai-docs/change_requests/<CR-ID-slug>/`.
3. If the user has source material to attach (a ticket export, a requirements PDF, mockup
   ...
   `.ai-docs/tools/pdf_to_md.py` to extract it to `requirements/requirements.md` +
   `requirements/images/`.
...
7. Follow `.ai-docs/coding-standards.md` conventions in every code snippet you write into these
   docs (PHP 7.0 — no nullable types, no `void` returns, no typed properties; `array()` syntax).
8. Use `.ai-docs/change_requests/CR-SET-66-Customs-support/` as the structural reference for
