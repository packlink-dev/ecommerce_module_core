...
> with [`coding-standard.md`](coding-standard.md) (how to write the code) and
...
| Path | Purpose |
|---|---|
| `docs/design.md` | System-wide architecture reference — layers, ORM, task queue, Packlink API proxy, business services, key flows, entity catalog, platform integration contract. |
| `docs/coding-standard.md` | Coding conventions derived from the codebase. |
| `docs/guidelines.md` | This file — the change-request workflow. |
| `docs/tools/` | Reusable scripts for authoring CR docs (e.g. `pdf_to_md.py` to convert a source requirements PDF to Markdown + extracted images). Not part of the documentation itself. |
| `docs/change-requests/<CR-ID>/` | One folder per change request (see below). |

## Change-request structure

Every change request gets its own folder under `docs/change-requests/`, named with the
...
- Raw source material the CR was derived from: the original ticket export, a requirements PDF,
  mockup images, or a `requirements.md` extracted from them (see `docs/tools/pdf_to_md.py`).
...
3. **Before implementing**, read `coding-standard.md` and the CR's `spec.md` + `design.md` + `tasks.md`.
...
5. **After implementing:**
   ...
   - Confirm PHP 7.0 compatibility — no nullable types (`?Type`), no `void` return types, no
     typed properties (see `coding-standard.md`; plain return type declarations like `: array`
